...
const evidence = path.resolve(extensionRoot, "../audit-shots/lens-everywhere-extension");
...
try {
  await context.route("https://lens.app/api/extension/library", (route) => route.fulfill({
    status: 200,
    contentType: "application/json",
    body: JSON.stringify({ operators: [], generators: [{ id: "evidence", name: "Evidence", summary: "", itemCount: 0 }] }),
  }));
  await context.route("https://lens.app/api/extension/execute", async (route) => {
  ...
  await panel.screenshot({ path: path.join(evidence, "01-persistent-page-highlight.png"), fullPage: true });
  ...
  await panel.screenshot({ path: path.join(evidence, "02-queued-explicit-go.png"), fullPage: true });
  ...
  await panel.screenshot({ path: path.join(evidence, "03-preview-before-insert.png"), fullPage: true });
  ...
  await page.screenshot({ path: path.join(evidence, "04-verified-field-insertion.png"), fullPage: true });

  fs.writeFileSync(path.join(evidence, "audit-results.json"), JSON.stringify({
    passed: 4,
    failed: 0,
    checks: ["persistent selection", "queue has no auto-run", "preview before mutation", "field insertion verification"],
    browser: await context.browser()?.version?.(),
    generatedAt: new Date().toISOString(),
  }, null, 2));
  console.log("Extension audit passed: 4 checks, 4 screenshots.");
} finally {
  await context.close();
  server.close();
  fs.rmSync(userDataDir, { recursive: true, force: true });
