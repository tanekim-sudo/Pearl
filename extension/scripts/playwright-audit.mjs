...
const evidence = path.resolve(extensionRoot, "../audit-shots/extension-distribution");
...
try {
  ...
  await panel.screenshot({ path: path.join(evidence, "01-library-import-preview.png"), fullPage: true });
  ...
  await panel.screenshot({ path: path.join(evidence, "02-persistent-page-highlight.png"), fullPage: true });
  ...
  await panel.screenshot({ path: path.join(evidence, "03-queued-explicit-go.png"), fullPage: true });
  ...
  await panel.screenshot({ path: path.join(evidence, "04-preview-before-insert.png"), fullPage: true });
  ...
  await page.screenshot({ path: path.join(evidence, "05-verified-field-insertion.png"), fullPage: true });

  fs.writeFileSync(path.join(evidence, "audit-results.json"), JSON.stringify({
    passed: 5,
    failed: 0,
    checks: ["library import preview", "persistent selection", "queue has no auto-run", "preview before mutation", "field insertion verification"],
    browser: await context.browser()?.version?.(),
    generatedAt: new Date().toISOString(),
  }, null, 2));
  fs.writeFileSync(path.join(evidence, "REPORT.md"), `# Extension distribution audit

- Public artifact: \`/downloads/lens-everywhere-chrome-v1.0.0.zip\`
- Library import preview: passed
...
The package is an unsigned Chrome MV3 developer-mode build. Chrome Web Store publication remains a separate signed-account action.
`);
  console.log("Extension audit passed: 5 checks, 5 screenshots.");
} finally {
  await context.close();
  server.close();
  fs.rmSync(userDataDir, { recursive: true, force: true });
