...
export function addCognitiveCandidates(request, candidates, { library = [], round = 1 } = {}) {
  ...
  for (const raw of candidates.slice(0, request.budget - request.candidates.length)) {
    ...
    accepted.push({
      ...
      purpose: raw.purpose || "",
      blurb: raw.blurb || "",
      steps: structuredClone(raw.steps || []),
      branches: structuredClone(raw.branches || []),
      inputSpec: structuredClone(raw.inputSpec || null),
      outputSpec: structuredClone(raw.outputSpec || null),
      examples: structuredClone(raw.examples || []),
      sourceIds: structuredClone(raw.sourceIds || []),
      labels: structuredClone(raw.labels || null),
      attribution: structuredClone(raw.attribution || null),
      alternatives: structuredClone(raw.alternatives || []),
      modelRecommendations: structuredClone(raw.modelRecommendations || []),
      holdouts: structuredClone(raw.holdouts || []),
      status: "proposed",
      review: [],
    });
  }
...
export function mergeCognitivePullRequest(request, { selectedCandidateIds, existingArtifacts = [] }) {
  ...
  const artifacts = candidates.map((candidate) => ({
    ...
    modelRecommendations: candidate.modelRecommendations,
    ...
    provenance: {
      ...
      derivedInterpretation: candidate.attribution?.derivedInterpretation === true,
    },
  }));
  return {
