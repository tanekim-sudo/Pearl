...
test("Companion and Studio remain explicit without simultaneous management clutter", () => {
  ...
  assert.match(studio, /Moves · Weights · Lenses/);
  assert.match(studio, /studio-structure-toggle/);
  assert.match(studio, /PearlFunctionMovesStudio/);
  assert.match(studio, /Organize into Moves → Weights → Lenses/);
  assert.match(extensionStudio, /studio-inspect|structureOpen|PearlFunctionMovesStudio|CognitiveLayerStudio/);
  const layerStudio = source("client/components/CognitiveLayerStudio.jsx");
  assert.match(layerStudio, /Moves → Weights → Lenses/);
  assert.match(layerStudio, /PEARL_STUDIO_COGNITIVE_SECTION_ORDER/);
  const movesStudio = source("client/components/PearlFunctionMovesStudio.jsx");
  assert.match(movesStudio, /studio-layer-weights/);
  assert.match(movesStudio, /studio-layer-moves/);
  assert.match(movesStudio, /studio-layer-lenses/);
